...
test_reg_service5() {
    ...
    for i in {2..5}; do
        ...
        ${!cleos} push action ${contract_oracle} regservice '{"account":"'${p}'", "base_stake_amount":"1000.0000 BOS", "data_format":"", "data_type":0, "criteria":"",
                          "acceptance":3, "declaration":"", "injection_method":0, "duration":20,
...
cleos1 push action ${contract_ocbc}  refund "consumer4444" -p ${contract_ocbc} 
cleos1 push action ${contract_ocbc}  undelegate "consumer4444" -p consumer4444
}
case "$1" in
