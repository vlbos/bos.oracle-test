...
reg_service(){
    cleos=cleos1 && if [ "$1" == "c2" ];then cleos=cleos2 ;fi
    ${!cleos}  push action  ${contract_oracle} regservice '{"service_id":"0", 
    "account":"eosio", 
    "stake_amount":"10.0000 EOS", "service_price":"1.0000 EOS",
                          "fee_type":"1", "data_format":"", "data_type":"", "criteria":"",
                          "acceptance":"0", "declaration":"", "injection_method":"0", "duration":"1",
                          "provider_limit":"3", "update_cycle":"1", "update_start_time":"0"}' -p eosio@active
...
cleos=cleos1 && if [ "$1" == "c2" ];then cleos=cleos2 ;fi
    ...
    ${!cleos}  push action  ${contract_oracle} addfeetypes '{"service_id":"0","fee_types":[0,1],"service_prices":["1.0000 EOS",".0000 EOS"] }' -p ${contract_oracle}@active
...
cleos=cleos1 && if [ "$1" == "c2" ];then cleos=cleos2 ;fi
    ...
    ${!cleos}  push action  ${contract_oracle} subscribe '{"service_id":"", 
    "contract_account":"", "action_name":"", "publickey":"",
                          "account":"", "amount":"", "memo":""}' -p ${contract_oracle}@active
}
...
cleos=cleos1 && if [ "$1" == "c2" ];then cleos=cleos2 ;fi
    ...
    ${!cleos} push action ${contract_oracle} push '{"service_id":"", "provider":"", "contract_account":"", "action_name":"",
                         "request_id":"", "data_json":""}' -p ${contract_oracle}
...
cleos=cleos1 && if [ "$1" == "c2" ];then cleos=cleos2 ;fi
      ...
      echo ===push
    ${!cleos}  push action  ${contract_oracle} push '{"service_id":"", "provider":"","data_json":"","is_request",""}' -p oracleoracle@active
}
...
cleos=cleos1 && if [ "$1" == "c2" ];then cleos=cleos2 ;fi
    ...
    ${!cleos}  push action  ${contract_oracle} disable '{"service_id":"",  "contract_account":"", "action_name":"",
